 ...
 * ...
 */
export const CustomTable = ({ columns, rows, maxWidth, ...properties }) => (
  <CustomTableContainer {...properties} maxWidth={maxWidth}>
    <thead>
      <tr>
        {columns.map((column) => (
          <th
        ...
        {rows.map((row) => (
          <tr key={row[0]}>
            {row.slice(1).map((value, index) => (
              <td
                key={columns[index].heading}
                data-label={columns[index].heading}
              >
                {value}
              </td>
            ))}
